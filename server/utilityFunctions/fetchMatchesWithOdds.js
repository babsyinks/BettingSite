...
//returned arr is of form [ [{},{}], [{},{}], etc ]
        ...
        teamsToBeMatched.forEach((tmsObj)=>{
          ...
          if(Math.sign(ratingsDiff) === 1){
            ...
            const trimmedMatch = match.replace(/\s+/g,'')

            ratingsObj[trimmedMatch] = `${hometeam.rating}-${awayteam.rating}`
    ...
    return {odds:teamsWithOdds(teamsToMatchTogether),ratings:ratingsObj}
 }
 ...
 function getMatches(teams,games){
    ...
    const mapGamesToOddsObjects = (games)=>{
       ...
       const mappedGames = games.map((game)=>{
            ...
            const homeTeamObj = teams.find((team)=>team.team === game.homeTeam)
            const awayTeamObj = teams.find((team)=>team.team === game.awayTeam)
            const match = `${game.homeTeam} vs ${game.awayTeam}`
            const trimmedMatch = match.replace(/\s+/g,'')
            ratingsObj[trimmedMatch] = `${homeTeamObj.rating}-${awayTeamObj.rating}`
        ...
        return {odds:flattenedMappedGames,ratings:ratingsObj}
    }
    ...
    if(games.length !== 0){
        ...
        if(games.length !== teams.length/2){
          ...
          const autoGeneratedGames = assignGamesWithOdds(arrOfTeams)
          const gamesFromFrontEnd = mapGamesToOddsObjects(games)
          return {
              odds:[...autoGeneratedGames.odds,...gamesFromFrontEnd.odds],
              ratings:{...autoGeneratedGames.ratings,...gamesFromFrontEnd.ratings}
        } 
        }
